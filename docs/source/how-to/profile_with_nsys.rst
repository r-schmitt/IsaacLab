...
The following command captures a profile for the ``Isaac-Cartpole-v0`` task via the ``rsl_rl`` training framework with 3 iterations:
...
The same wrapper works for the benchmark entry points under ``scripts/benchmarks/``. For
example, to profile the vision Shadow Hand task through the training benchmark:
...
Because the domain swim-lanes are keyed by module/function, the resulting ``.nsys-rep``
lines up lane-for-lane with profiles captured on other Isaac Lab branches for the shared
code paths, which makes cross-branch A/B comparison straightforward.
...
.. note::
   ...
   Never trace USD schema classes (``pxr.UsdGeom.Mesh`` and friends) or ``@classmethod`` /
   ``@staticmethod`` members. nsys replaces the traced attribute with a plain wrapper
   function, which breaks ``TfType`` registration for schema classes and destroys descriptor
   binding for class/static methods. See the ``_comment`` fields in the JSON for details.
