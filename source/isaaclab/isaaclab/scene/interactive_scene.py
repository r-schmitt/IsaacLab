    ...
    * For tasks that require having separate assets in the environments, ``replicate_physics`` would have to
    ...
    def physics_scene_path(self) -> str:
        ...
        if self._physics_scene_path is None:
            ...
            #
            # NOTE: This intentionally walks the whole stage rather than resolving
            # ``sim.cfg.physics_prim_path`` directly. Under OvPhysX the per-environment
            # clones are populated lazily into USD from Fabric, and this traversal is
            # what forces that materialization before USD-reading sensors (camera,
            # ray caster, contact) build their views. Short-circuiting it leaves only
            # ``env_0`` authored, so a camera view finds 1 prim instead of ``num_envs``
            # and simulation start fails. Do not replace this with a direct lookup
            # until clone materialization is triggered explicitly elsewhere.
